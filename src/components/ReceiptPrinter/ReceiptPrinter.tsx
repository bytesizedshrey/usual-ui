...
const machineClassName =
  "relative isolate w-full overflow-hidden rounded-[var(--printer-radius)] border border-black/50 bg-gradient-to-b from-[#202020] to-[#191919] p-[var(--printer-inset)] pb-8 shadow-[0_1px_0.5px_#ffffff1a_inset,0_1px_1px_#ffffff35_inset,0_10px_10px_-9px_#00000070,0_20px_20px_-14px_#00000060,0_0px_6px_0px_#00000060] [--printer-inner-radius:calc(var(--printer-radius)_-_var(--printer-inset))] [--printer-inset:0.75rem] [--printer-radius:1.5rem] before:pointer-events-none before:absolute before:inset-0 before:z-0 before:rounded-[inherit] before:bg-[url('/textures/plastic-noise.svg')] before:bg-[length:180px_180px] before:bg-repeat before:opacity-20 before:mix-blend-overlay before:content-['']";
...
function ReceiptPrinterMachine({
  ...
  return (
    <div className={cn(machineClassName, className)} {...props}>
      ...
      <div
        aria-hidden="true"
        className="absolute inset-x-6 bottom-[var(--printer-inset)] z-40 h-2 rounded-full bg-[#0a0a0a] shadow-[0_0.5px_0_#ffffff50,0_2px_6px_#00000090_inset]"
      />
    </div>
  );
}
...
function ReceiptPrinterScreen({
  ...
  return (
    <div
      className={cn(
        "relative z-10 isolate overflow-hidden rounded-[var(--printer-inner-radius)] bg-[#111111] p-4 text-[#e7e4dc] shadow-[0_0.5px_0_#ffffff50,0_2px_6px_#00000090_inset] after:pointer-events-none after:absolute after:inset-0 after:z-20 after:rounded-[inherit] after:shadow-[inset_0_0_24px_4px_#00000080] after:content-['']",
        className,
      )}
      {...props}
    >
...
function StatusIndicator({
  ...
  return (
    <span
      aria-hidden="true"
      className="relative grid size-6 shrink-0 place-items-center rounded-full bg-[#0a0a0a] shadow-[0_0.5px_0_#ffffff50,0_2px_6px_#00000090_inset]"
    >
      <AnimatePresence initial={false} mode="sync">
        {isComplete ? (
          ...
          <motion.span
            animate={{ opacity: 1, transform: "scale(1)" }}
            className="col-start-1 row-start-1 grid place-items-center text-[#8f8b82]"
            exit={{
              opacity: animate ? 0 : 1,
              transform: move ? "scale(0.96)" : "scale(1)",
            }}
      ...
      >
        <AnimatePresence initial={false} mode="sync">
          <motion.div
            ...
            className="col-start-1 row-start-1 truncate font-medium text-[#8f8b82] text-sm leading-none"
            exit={{
              opacity: animate ? 0 : 1,
...
function ReceiptPrinterPaper({
  ...
  return (
    <article
      className={cn(
        "relative z-10 min-h-80 bg-[#f3ede2] bg-[url('/textures/receipt-paper.svg')] bg-cover px-6 pt-7 pb-8 font-mono text-[#2a2420] bg-blend-soft-light shadow-[0_1px_2px_#00000030]",
        className,
      )}
  ...
  return (
    ...
    >
      {isReceiptVisible ? (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-6 -top-1 z-20 h-2 bg-black/70 blur-[6px]"
        />
      ) : null}
      ...
      <motion.div
        ...
        className="relative isolate before:pointer-events-none before:absolute before:inset-x-3 before:top-3 before:bottom-4 before:z-0 before:rounded-sm before:shadow-[0_8px_24px_#00000060] before:content-[''] after:pointer-events-none after:absolute after:right-[8%] after:bottom-0 after:left-[8%] after:z-0 after:h-3 after:translate-y-1.5 after:rounded-full after:bg-black/15 after:blur-lg after:content-['']"
        initial={false}
        transition={{
