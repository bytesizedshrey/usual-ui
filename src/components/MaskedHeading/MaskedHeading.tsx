...
export interface MaskedHeadingProps {
  ...
  weight?: number;
  tracking?: number;
  /** Extra spacing between words, in em (scales with the fluid, JS-computed font size). Independent of `tracking`, which only affects letter spacing. */
  wordGap?: number;
  lineHeight?: number;
  textScale?: number;
  className?: string;
...
const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(root.clientWidth * s.textScale, 44, 200).toFixed(1)}px`;
  ...
  useEffect(() => {
    sync();
  }, [sync, words, tag, align, weight, tracking, wordGap, lineHeight, textScale]);
