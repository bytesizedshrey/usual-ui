...
export type UsePopoverOptions = {
  ...
  /** Caps the content's preferred height as a fraction of viewport height, so it doesn't grow to fill all available room even when there's plenty. */
  maxContentHeightRatio?: number;
  /** Further caps the preferred height (post-ratio) in pixels. */
...
export function usePopover<A extends HTMLElement = HTMLElement>({
  ...
  maxContentHeightRatio,
  maxContentHeightPx,
}: UsePopoverOptions): UsePopoverResult<A> {
  const internalAnchorRef = useRef<A>(null);
  const anchorRef = (externalAnchorRef as React.RefObject<A | null> | undefined) ?? internalAnchorRef;
  ...
  const update = useCallback(() => {
    ...
    // Clearing max-height below temporarily un-clips the content to measure
    // its natural size, which clamps scrollTop to 0 in the interim — save it
    // so a scrolled panel doesn't jump back to the top on every remeasure
    // (e.g. triggered by animated content resizing inside the panel).
    const savedScrollTop = content?.scrollTop ?? 0;
    ...
    if (content) {
      ...
      let maxContentHeight = Math.max(MIN_H, allowed - chrome);
      if (maxContentHeightRatio !== undefined) {
        const preferred = Math.min(
          vh * maxContentHeightRatio,
          maxContentHeightPx ?? Infinity,
        );
        maxContentHeight = Math.min(maxContentHeight, Math.max(MIN_H, preferred - chrome));
      }
  ...
  }, [side, align, offset, padding, arrowSize, boundary, maxContentHeightRatio, maxContentHeightPx]);
  ...
  useEffect(() => {
    ...
    const onScroll = (event: Event) => {
      // Scrolling inside the panel's own content shouldn't trigger a
      // reposition/remeasure — only the page (or another ancestor) scrolling
      // should. Otherwise remeasuring clears and reapplies max-height on
      // every tick, which resets the content's scrollTop back to 0.
      const target = event.target;
      if (
        target instanceof Node &&
        (contentRef.current?.contains(target) || panelRef.current?.contains(target))
      ) {
        return;
      }
      schedule();
...
export function Popover({
  ...
  // Manually drive scrolling of the content area. React attaches wheel
  // listeners as passive, so a plain onWheel handler can't rely on
  // preventDefault; a native, non-passive listener guarantees the panel's
  // content scrolls under the mouse wheel regardless of the page's other
  // event listeners or animated content nearby.
  useEffect(() => {
    if (!open) return;
    const content = contentRef.current;
    if (!content) return;
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex h-9 select-none items-center gap-2 rounded-[9px] border border-white/[0.16] bg-[#1D1D1A] px-3 text-[13px] font-medium text-stone-200 outline-none transition-[border-color,box-shadow] duration-150 hover:border-white/20 focus-visible:border-[#93B0FF] focus-visible:shadow-[0_10px_20px_-14px_rgba(147,176,255,0.5)] ${triggerClassName}`}
      >
        {trigger}
      </button>
              ...
              {open ? (
                ...
                >
                  <motion.div
                    ...
                    className={`relative rounded-[11px] border border-white/[0.16] bg-[#0a0a0a] p-3 shadow-[0_18px_40px_-24px_rgba(0,0,0,0.9)] focus-visible:outline-none ${className}`}
                  >
                    <span
                      ref={arrowRef}
                      aria-hidden
                      style={{ width: arrowSize, height: arrowSize, transform: "rotate(45deg)" }}
                      className={`absolute block bg-[#0a0a0a] border-white/[0.16] ${ARROW_EDGE[at]}`}
                    />
